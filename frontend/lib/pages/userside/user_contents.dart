...
import 'package:frontend/pages/userside/view_content.dart';
...
class UserContents extends StatefulWidget {
  ...
  const UserContents({super.key, required this.category});
...
class _UserContentsState extends State<UserContents> {
  ...
  Future<void> _getContents() async {
    try {
      final url = Uri.parse('$backendUrl/edu/userContents');
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        final List<dynamic> responseBody =
            jsonDecode(response.body)['userContents'];
        setState(() {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ViewContent(contentId: contentId),
        ),
      ),
      child: Column(
        children: [
